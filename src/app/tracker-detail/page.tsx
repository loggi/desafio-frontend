      ...
      {Object.values(data).map((d, index) : React.ReactNode => (
        <div className="bg-white div--info flex flex-col h-full ml-40 p-8 rounded-2xl" key={ index }>
          <div className="div--status flex flex-col p-5 rounded-t-2xl">
            <span className="font-medium font-sans inline-block text-base text-white">{ d[0].status }</span>
            <span className="font-sans inline-block text-base text-white opacity-80">{ d[0].status_description }</span>
          </div>
          ...
          <div className="flex flex-row pb-4 pt-0">
            <div className="border-r flex flex-col pb-2 px-5 w-1/2">
              ...
              <span className="font-sans span--gray text-sm">{ d[0].receiver }</span>
            </div>
